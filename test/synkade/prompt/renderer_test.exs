defmodule Synkade.Prompt.RendererTest do
  ...
  describe "render_custom" do
    test "interpolates issue variables" do
      template = "Work on {{ issue.identifier }}: {{ issue.title }}"
      assert {:ok, rendered} = Renderer.render_custom(template, @project, @issue)
    ...
    test "interpolates attempt variable" do
      ...
      assert {:ok, rendered} = Renderer.render_custom(template, @project, @issue, 2)
    ...
    test "returns error for invalid template syntax" do
      ...
      assert {:error, {:template_parse_error, _}} = Renderer.render_custom(template, @project, @issue)
    end
  end
